...
# ================= CONFIG =================
# ARDEN STORE VARIANT of the OnBuy-eBay pipeline. Key difference from the
...
# ================= ONBUY API PUSH (permanently OFF for Arden) =================
# The Arden store updates OnBuy manually via CSV export - by explicit policy,
...
def sku_numeric_part(sku):
    ...
    the barcode ("ARD-5012345678900") and everything validated, exported or
...
def main():
    ...
    if missing or duplicates:
        ...
        notify.send_alert_email("Arden sheet header row needs fixing", message)
        sys.exit(1)
    ...
    # "ARD-5012345678900" and "5012345678900" count as duplicates. The FIRST
    ...
    for idx, row in batch:
        ...
        # ================= CHANGE ALERTS (Arden manual-update workflow) =====
        ...
        _FLAG_PREFIXES = ("variant", "choose variant", "unreadable link", "add sku",
                          "invalid sku", "duplicate", "aliexpress", "protected")
        ...
        if change_notes:
            ...
            alert_for_row = ""
            alert_time_for_row = ""
        elif existing_alert and supplier == "AliExpress" and existing_alert.upper().startswith("PRICE"):
            # Leftover from before the 2026-07-21 policy silenced AliExpress
            # price alerts: the sheet price has moved on since, so the stale
            # "PRICE A -> B" would have OnBuy set to an outdated number.
            # Stock alerts (and mixed alerts, which start with the stock
            # text) still wait for the Applied tick.
            alert_for_row = ""
            alert_time_for_row = ""
        else:
        ...
        # Change-alert columns (Arden manual-update workflow). "Applied on
        ...
        supabase_row = {
            ...
            # Arden change-alert state, mirrored so Supabase holds the same
    ...
    # ================= CHANGE-ALERT EMAIL (Arden manual-update workflow) =================
    ...
    if change_log:
        notify.send_alert_email(
            f"Arden: {len(change_log)} product change(s) need action on OnBuy",
            "These products changed at the supplier and need a manual update on OnBuy:\n\n"
            + "\n".join(f"- SKU {sku}: {note}" for sku, note in change_log)
            + "\n\nThe same rows are highlighted in the Arden sheet (amber = change pending, "
